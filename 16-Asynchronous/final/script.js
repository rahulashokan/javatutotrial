...
a new image (use document.createElement('img')) and 
...
4. After the 2 seconds have passed, 
   hide the current image (set display to 'none'), and 
   load a second image (HINT: Use the image element returned by 
   the createImage promise to hide the current image. 
...
const loadNPause = async function () {
  try {
    ...
    console.log('Asyc Image 1 loaded');
    await wait(1);
    ...
    console.log('Asyc Image 2 loaded');
    await wait(1);
    img.style.display = 'none';
  } catch (error) {
    console.error(error);
...
const loadAll = async function (imgArr) {
  try {
    ...
    const load = await Promise.all(imgs);
    console.log(load);
    load.forEach(img => img.classList.add('parallel'));
  } catch (error) {
    console.log(error);
  }
};
